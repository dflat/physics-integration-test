...
void CharacterSystem::Register(World &world) {
...
void CharacterSystem::Update(World &world, float dt) {
  ...
  world.each<CharacterHandle, PlayerInput, PlayerState, WorldTransform>(
      [&](Entity e, CharacterHandle &h, PlayerInput &input, PlayerState &state, WorldTransform &wt) {
        ...
        if (input.jump && can_jump) {
            vertical_vel = (state.jump_count == 0) ? 12.0f : 10.0f;
        ...
        if (horizontal_vel.LengthSq() > 0.1f) {
            ...
            float target_angle = atan2f(look_dir.GetX(), look_dir.GetZ());
            JPH::Quat target_rot = JPH::Quat::sRotation(JPH::Vec3::sAxisY(), target_angle);
